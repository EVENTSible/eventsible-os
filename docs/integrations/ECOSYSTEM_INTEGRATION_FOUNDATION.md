...
- Verification runner: `scripts/ecosystem-local-supabase-verify.mjs`
- Guard runner: `scripts/guard-local-supabase-ci.mjs`

Reason for CI-local verification:

- Codex sandbox workers could not access Desktop Docker or WSL even after Docker Desktop 4.84.0 and `npx supabase --version` 2.111.0 worked in normal PowerShell.
- Supabase cloud staging remained unavailable from prior attempts because the existing Production project ref `cplpbzudjprzbnzocirc` could not create the requested development branch and no additional paid project should be created for this phase.
- The GitHub Actions workflow uses an Ubuntu hosted runner, starts a local Supabase Docker stack, applies repository migrations from zero, runs synthetic integration assertions, and stops the local stack in an always-run cleanup step.

Safety boundaries:

- The workflow does not use Production Supabase secrets.
- It does not call `supabase link`, remote `db push`, Vercel commands, ECC/VINCE commands, or any Production Supabase API.
...
- The local database must build from zero using repository migrations only.
- Required local objects include `public.os_ingest_builder_submission`, `os_contacts`, `os_builder_submissions`, `os_leads`, `os_events`, `os_quote_versions`, `os_quote_items`, `os_builder_activity`, `os_integration_outbox`, indexes, RLS, service-role policies, and helper functions for known service mapping and public catalog conversion.
- Missing or out-of-order schema should fail the workflow instead of being patched inside the workflow.

Synthetic intake scenario:

- Event type: Private Party.
- Planning stage: Ready for a quote.
- Date: future Tuesday generated by the CI test.
- Length: 3 hours.
- Location: South Bend, Indiana.
- Goals: Packed Dance Floor, Photos & Video, Guest Interaction.
- Known services: DJ/MC, Selfie Booth with Prints, Live Singer, Event Staff.
- Controlled unknown service: `unknown-synthetic-service`.
...
Expected ID and contract assertions:

- One `event_id` links the first Contact, Builder submission, Lead, Quote draft/version, Quote items, Builder activity, and Outbox event.
- The second distinct submission creates a different `event_id` and a separate linked chain.
- `contract_version` is `builder_submission_v1`.
- Quote metadata records `quote_draft_v1`; public catalog conversion records `public_service_catalog_v1`.
- `source` is `eventsible_event_builder`; outbox `source_application` is `event_builder`.
- The idempotency key is the Builder submission ID.

Expected totals and mapping:

- UI/OS quote total: `66700` cents.
- Package savings: `6300` cents.
- Travel: `0` cents.
- Known service codes: `dj_mc`, `selfie_booth_prints`, `live_performer`, and `event_staff`.
- Selfie Booth with Prints remains human-readable.
- Live Singer remains Custom Quote and contributes `0` cents to numeric totals.
- The unknown synthetic service remains Custom Quote and contributes `0` cents to numeric totals.

Expected outbox and RLS assertions:

- `os_integration_outbox` contains event type, payload version, related IDs, idempotency key, status, retry count, failure history, and timestamps.
- Outbox payloads include record IDs and contract metadata but avoid secrets and unnecessary contact fields.
- Anon role cannot read `os_integration_outbox`.
- Anon role cannot insert arbitrary outbox events.
- Anon role cannot read `os_contacts`.
- Local service-role execution can call `public.os_ingest_builder_submission`.

Expected catalog assertions:

- Public catalog conversion includes contract version, stable service ID, public name, public description, public pricing, minimum hours, weekday rules, Custom Quote status, public media, and active status.
- Public catalog conversion excludes internal costs, margins, partner rates, internal notes, private staff notes, private equipment notes, and server-only metadata.
- Production Builder is not cut over to the OS catalog in this phase.

Expected failure and rollback assertions:

- Invalid contact, missing service selections, invalid contract version, oversized notes, honeypot, unexpected privileged fields, duplicate submission, forced database failure, and transaction rollback paths are tested.
- Failures must not return false success, leave partial chains, leak SQL details, expose stack traces, or expose secrets.

CI standard checks:

- `npm install`.
- `npm run test`.
- `npm run lint:ci`, which runs `npm run lint` and only tolerates the inherited `public/gigtracker-v1.js` parse error while printing it clearly.
- `npm run build`.
- `npm audit`, allowed to report inherited high advisories without forcing risky upgrades.

Remaining difference from cloud Preview verification:

- CI-local verification proves migrations and integration behavior from a clean local database with synthetic data.
- It does not prove that the authorized Preview or Production Supabase database has already received the migration.
- It does not prove deployed Builder Preview can write to the cloud OS database.
- It does not replace authenticated Admin visual QA.

Production migration prerequisites:

1. Review the passing GitHub Actions run and exact CI row-count summary.
2. Confirm the target Supabase project is not EventsGame and is not accidentally selected by local CLI state.
3. Apply only reviewed additive migrations through the approved Supabase migration process.
4. Run one controlled Preview Builder submission with authorized database readback.
5. Verify exact one-chain row counts, idempotent replay, second distinct submission, outbox/RLS, catalog privacy, Admin display, and no secret exposure.
6. Request explicit approval before merge or Production promotion.
