...
| --- | --- | --- | --- |
| Event Builder | PRODUCTION VERIFIED | `build.eventsible.info` returned 200 after the documentation merge; Vercel production deployment metadata confirms a READY production build on the merged `main` documentation commit. The previous known live application checkpoint remains `dpl_7C2n8eBUoWuDDM6N7kjwcd8cog84` on commit `ab128cefb9bf97042dba0f3222ecbca858b0f863`. | Staff Admin Leads visual QA on production still requires a fresh authenticated staff session. |
| EVENTSible OS / GigTracker | IMPLEMENTED / PARTIAL / NEEDS DOMAIN VERIFICATION | Repo `eventsible-os-admin` is the canonical documentation hub in `EVENTSible/eventsible-os`; Vercel production deployment metadata confirms READY documentation-triggered production builds on merged `main` commits. Source includes admin, client portal assets, event workspace, intake helper, docs. | `eventsible.biz` and `client.eventsible.biz` routing/auth must be verified. `eventsible-os.vercel.app` returned 404 in a safe route check on 2026-07-29. |
| ECC / VINCE | IMPLEMENTED; production verified through current deployment metadata and health route | Repo `EVENTSible/ecc-vince`; Vercel production deployment metadata confirms a READY production build on the merged `master` documentation commit; `/api/health` returned OK on 2026-07-29; public room routes serve the app shell. | Deep hydrated browser QA for Player/Audience/Host was not repeated in this documentation-only pass. Do not enter Host PIN without explicit action-time authorization. |
