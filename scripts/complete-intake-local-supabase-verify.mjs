...
if(execute(`select count(*) from public.os_event_notes where body='Historical organizer: Synthetic Organizer (provenance only; no team account).'`)!=="1"||execute("select count(*) from public.os_team_members where display_name='Synthetic Organizer'")!=="0")throw new Error("Historical organizer provenance created an operational team account or lost its note.");
