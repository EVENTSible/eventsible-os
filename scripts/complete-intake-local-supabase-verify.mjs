...
if(execute("select exists(select 1 from supabase_migrations.schema_migrations where version='20260916194052')")!=="t")throw new Error("Financial-integrity importer migration is absent from the isolated ledger.");
...
if(execute("select count(*) from information_schema.columns where table_schema='public' and table_name='os_booking_payment_facts' and column_name in ('contracted_or_quoted_amount','deposit_received_amount','tip_amount','overtime_amount','balance_due_amount')")!=="5")throw new Error("Dedicated financial evidence columns are incomplete.");
if(execute("select exists(select 1 from information_schema.columns where table_schema='public' and table_name='os_events' and column_name='historical_date' and data_type='date')")!=="t")throw new Error("Date-only event storage is unavailable.");
if(execute("select count(*) from pg_indexes where schemaname='public' and indexname='os_events_historical_date_idx'")!=="1")throw new Error("Date-only duplicate-detection index is missing.");
if(execute("select bool_and(p.prosecdef and r.rolname='postgres' and 'search_path=\"\"'=any(p.proconfig)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace join pg_roles r on r.oid=p.proowner where n.nspname='private' and p.proname in ('os_validate_complete_import_evidence_item','os_populate_complete_import_payment_fact','os_correct_complete_import_booking_financials','os_populate_complete_import_historical_date')")!=="t")throw new Error("Financial-integrity trigger functions are not controlled SECURITY DEFINER routines.");
...
function buildManifest({suffix="success",badTeam=false,duplicateEmail=null}={}){
  const clientIndexes=Array.from({length:24},(_,i)=>i).filter((i)=>![20,21].includes(i));
  const contacts=clientIndexes.map((i)=>({key:`contact.${suffix}-${i}`,type:"contact",sourceHash:sourceHash(`${suffix}:contact:${i}`),sourceRef:`synthetic/${suffix}/contact-${i}`,uncertainFields:[],data:{displayName:`Synthetic contact ${suffix} ${i}`,primaryEmail:duplicateEmail&&i===0?duplicateEmail:`${suffix}-${i}@example.invalid`}}));
  const contactByIndex=new Map(contacts.map((contact)=>[Number(contact.key.split("-").at(-1)),contact]));
  const events=Array.from({length:24},(_,i)=>({key:`event.${suffix}-${i}`,type:"event",sourceHash:sourceHash(`${suffix}:event:${i}`),sourceRef:`synthetic/${suffix}/event-${i}`,uncertainFields:[],data:{...(contactByIndex.has(i)?{primaryContactItemKey:contactByIndex.get(i).key}:{}),title:`Synthetic gig ${suffix} ${i}`,eventType:i===20?"vendor_market":i===21?"community_event":i===23?"cross_country":"test",status:i===20?"completed":i===21?"active":i===22?"inquiry":i===23?"pending":"completed",recordDisposition:i===20?"vendor_appearance":i===21?"operational_event":i===22?"lower_confidence_review":i===23?"pending_unbooked":"confirmed",...(i===19?{historicalDate:"2027-08-19"}:{startsAt:`2027-0${(i%9)+1}-01T15:00:00Z`,endsAt:`2027-0${(i%9)+1}-01T18:00:00Z`,timezone:"America/Indiana/Indianapolis"})}}));
  const inquiries=[22,23].map(i=>({key:`inquiry.${suffix}-${i}`,type:"inquiry",sourceHash:sourceHash(`${suffix}:inquiry:${i}`),sourceRef:`synthetic/${suffix}/inquiry-${i}`,uncertainFields:[],data:{contactItemKey:contactByIndex.get(i).key,eventItemKey:events[i].key,status:"new",summary:i===23?"Synthetic pending, unbooked, no deposit":"Synthetic lower-confidence review"}}));
  const bookings=Array.from({length:20},(_,i)=>({key:`booking.${suffix}-${i}`,type:"booking",sourceHash:sourceHash(`${suffix}:booking:${i}`),sourceRef:`synthetic/${suffix}/booking-${i}`,uncertainFields:[],data:{eventItemKey:events[i].key,status:"completed",contractStatus:"signed",bookedAt:"2026-09-01T12:00:00Z"}}));
  ...
  const paymentEvidence=(i)=>{
    if(i===0)return {contractedOrQuotedValue:"595",grossClientAmount:"595",platformFeeAmount:"14.88",netPayoutAmount:"580.12",paymentMethod:"gigsalad",paymentStatus:"paid",payoutStatus:"paid",balanceDue:"0"};
    if(i===1)return {contractedOrQuotedValue:"1025",grossClientAmount:"265",depositAmount:"265",platformFeeAmount:"0",netPayoutAmount:"265",paymentMethod:"invoice",paymentStatus:"deposit_paid",payoutStatus:"not_applicable",balanceDue:"760"};
    if(i===2)return {contractedOrQuotedValue:"525",grossClientAmount:"300",depositAmount:"300",platformFeeAmount:"0",netPayoutAmount:"300",paymentMethod:"invoice",paymentStatus:"deposit_paid",payoutStatus:"not_applicable",balanceDue:"225"};
    if(i===3)return {contractedOrQuotedValue:"485",grossClientAmount:"125",depositAmount:"125",platformFeeAmount:"0",netPayoutAmount:"125",paymentMethod:"invoice",paymentStatus:"deposit_paid",payoutStatus:"not_applicable",balanceDue:"360"};
    if(i===4)return {grossClientAmount:"145",depositAmount:"145",platformFeeAmount:"0",netPayoutAmount:"145",paymentMethod:"gigsalad",paymentStatus:"deposit_paid",payoutStatus:"pending"};
    if(i===5)return {contractedOrQuotedValue:"500",grossClientAmount:"600",tipAmount:"100",platformFeeAmount:"0",netPayoutAmount:"600",paymentMethod:"check",paymentStatus:"paid",payoutStatus:"not_applicable",balanceDue:"0"};
    return {contractedOrQuotedValue:"500",grossClientAmount:"500",platformFeeAmount:"0",netPayoutAmount:"500",paymentMethod:"check",paymentStatus:"paid",payoutStatus:"not_applicable",balanceDue:"0"};
  };
  const payments=bookings.map((booking,i)=>({key:`payment.${suffix}-${i}`,type:"payment_fact",sourceHash:sourceHash(`${suffix}:payment:${i}`),sourceRef:`synthetic/${suffix}/payment-${i}`,uncertainFields:[],data:{bookingItemKey:booking.key,...paymentEvidence(i),currency:"USD"}}));
  ...
  const notes=events.map((event,i)=>({key:`note.${suffix}-${i}`,type:"operational_note",sourceHash:sourceHash(`${suffix}:note:${i}`),sourceRef:`synthetic/${suffix}/note-${i}`,uncertainFields:[],data:{eventItemKey:event.key,noteType:i===21?"staff":"general",body:i===21?"Historical organizer: Synthetic Organizer (provenance only; no team account).":`Synthetic operational note ${i}`}}));
...
const ordinaryWithoutContact=buildManifest({suffix:"ordinary-without-contact"});
ordinaryWithoutContact.items.find((item)=>item.key==="event.ordinary-without-contact-20").data.recordDisposition="confirmed";
ordinaryWithoutContact.itemCounts=completeItemCounts(ordinaryWithoutContact.items);
const ordinaryRaw=JSON.stringify(ordinaryWithoutContact);const ordinaryHash=createHash("sha256").update(ordinaryRaw).digest("hex");
denied(ids.owner,"owner",`select public.os_stage_complete_intake_manifest('${Buffer.from(ordinaryRaw).toString("base64")}','${ordinaryHash}',24,'${sqlText(JSON.stringify(ordinaryWithoutContact.itemCounts))}'::jsonb)`);

const commercialOperational=buildManifest({suffix:"commercial-operational"});
commercialOperational.items.push({key:"booking.commercial-operational-20",type:"booking",sourceHash:sourceHash("commercial-operational:booking:20"),sourceRef:"synthetic/commercial-operational/booking-20",uncertainFields:[],data:{eventItemKey:"event.commercial-operational-20",status:"confirmed",contractStatus:"signed"}});
commercialOperational.itemCounts=completeItemCounts(commercialOperational.items);
const commercialRaw=JSON.stringify(commercialOperational);const commercialHash=createHash("sha256").update(commercialRaw).digest("hex");
denied(ids.owner,"owner",`select public.os_stage_complete_intake_manifest('${Buffer.from(commercialRaw).toString("base64")}','${commercialHash}',24,'${sqlText(JSON.stringify(commercialOperational.itemCounts))}'::jsonb)`);
...
const expectedCanonical={contacts:22,events:24,bookings:20,services:20,payments:20,assignments:3,notes:24,provenance:24};
...
if(execute(`select count(*) from public.os_events where settings->>'importBatchId'='${staged.batchId}' and primary_contact_id is null and settings->>'contactRelationship'='none_operational' and settings->>'recordDisposition' in ('vendor_appearance','operational_event')`)!=="2")throw new Error("Contactless operational events were not created with the bounded classification.");
if(execute(`select count(*) from public.os_event_notes n join public.os_import_batch_items i on (i.canonical_record_ids->>'primaryId')::uuid=n.id where i.batch_id='${staged.batchId}' and i.item_key='note.success-21' and n.body='Historical organizer: Synthetic Organizer (provenance only; no team account).'`)!=="1"||execute("select count(*) from public.os_team_members where display_name='Synthetic Organizer'")!=="0")throw new Error("Historical organizer provenance created an operational team account or lost its batch-scoped note.");
const operationalEventId=execute(`select id from public.os_events where settings->>'importBatchId'='${staged.batchId}' and settings->>'recordDisposition'='vendor_appearance'`);
execute(`insert into public.os_bookings(event_id,status,contract_status,payment_status) values('${operationalEventId}','pending','not_sent','unpaid')`,{expectFailure:true});
if(execute(`select count(*) from public.os_bookings where event_id='${operationalEventId}'`)!=="0")throw new Error("A contactless operational event accepted a later booking.");
if(execute(`select contracted_or_quoted_amount||':'||gross_client_amount||':'||coalesce(deposit_received_amount::text,'NULL')||':'||platform_fee_amount||':'||net_payout_amount||':'||payment_method||':'||payment_status||':'||payout_status from public.os_booking_payment_facts where source_ref='synthetic/success/payment-0'`)!=="595.00:595.00:NULL:14.88:580.12:gigsalad:paid:paid")throw new Error("Gross, fee, net, and contract evidence were not preserved separately from an unsupported deposit.");
for(const [index,total,deposit,balance] of [[1,"1025.00","265.00","760.00"],[2,"525.00","300.00","225.00"],[3,"485.00","125.00","360.00"]]){
  if(execute(`select b.total_amount||':'||b.deposit_amount||':'||b.balance_due from public.os_bookings b join public.os_import_batch_items i on (i.canonical_record_ids->>'primaryId')::uuid=b.id where i.batch_id='${staged.batchId}' and i.item_key='booking.success-${index}'`)!==`${total}:${deposit}:${balance}`)throw new Error(`Deposit regression ${index} overwrote the reviewed contract value.`);
}
if(execute(`select coalesce(b.total_amount::text,'NULL')||':'||b.deposit_amount from public.os_bookings b join public.os_import_batch_items i on (i.canonical_record_ids->>'primaryId')::uuid=b.id where i.batch_id='${staged.batchId}' and i.item_key='booking.success-4'`)!=="NULL:145.00")throw new Error("Deposit-only evidence was promoted to a booking total.");
if(execute(`select contracted_or_quoted_amount||':'||gross_client_amount||':'||coalesce(deposit_received_amount::text,'NULL')||':'||tip_amount||':'||platform_fee_amount from public.os_booking_payment_facts where source_ref='synthetic/success/payment-5'`)!=="500.00:600.00:NULL:100.00:0.00")throw new Error("Tip evidence was merged into contract, deposit, or fee fields.");
if(execute(`select historical_date||':'||coalesce(starts_at::text,'NULL')||':'||coalesce(ends_at::text,'NULL')||':'||coalesce(timezone,'NULL') from public.os_events where settings->>'importBatchId'='${staged.batchId}' and title='Synthetic gig success 19'`)!=="2027-08-19:NULL:NULL:NULL")throw new Error("A date-only historical event lost its date or gained a fabricated time or timezone.");
