...
if(execute("select exists(select 1 from supabase_migrations.schema_migrations where version='20260915223726')")!=="t")throw new Error("Contactless operational importer migration is absent from the isolated ledger.");
...
function buildManifest({suffix="success",badTeam=false,duplicateEmail=null}={}){
  ...
  const events=Array.from({length:24},(_,i)=>({key:`event.${suffix}-${i}`,type:"event",sourceHash:sourceHash(`${suffix}:event:${i}`),sourceRef:`synthetic/${suffix}/event-${i}`,uncertainFields:[],data:{...(contactByIndex.has(i)?{primaryContactItemKey:contactByIndex.get(i).key}:{}),title:`Synthetic gig ${suffix} ${i}`,eventType:i===20?"vendor_market":i===21?"community_event":i===23?"cross_country":"test",status:i===20?"completed":i===21?"active":i===22?"inquiry":i===23?"pending":"completed",recordDisposition:i===20?"vendor_appearance":i===21?"operational_event":i===22?"lower_confidence_review":i===23?"pending_unbooked":"confirmed",startsAt:`2027-0${(i%9)+1}-01T15:00:00Z`,endsAt:`2027-0${(i%9)+1}-01T18:00:00Z`,timezone:"America/Indiana/Indianapolis"}}));
