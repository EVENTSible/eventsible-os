...
import { applyVeraWarrenAction, applyWarrenOwnerAttestationAction, archiveCleanupCustomersAction, compensateBatchAction, compensateVeraWarrenAction, compensateWarrenOwnerAttestationAction, applyBatchAction, approveBatchAction, manageContactAction, manageEventAction, manageLeadAction, previewCleanupAction, previewVeraWarrenAction, previewWarrenOwnerAttestationAction, quarantineCleanupOutboxAction, restoreCleanupCustomersAction, restoreCleanupOutboxAction, stageManifestAction } from "./actions";
...
export default async function DataReadinessPage(){
  ...
  const snapshotData=(snapshot.data && typeof snapshot.data==="object" ? snapshot.data : {}) as {contacts?:Record<string,unknown>[];events?:Record<string,unknown>[];leads?:Record<string,unknown>[];services?:Record<string,unknown>[];bookings?:Record<string,unknown>[];paymentFacts?:Record<string,unknown>[];batches?:Record<string,unknown>[];items?:Record<string,unknown>[];activity?:Record<string,unknown>[]};
  ...
  return <RecordsIntakeWorkspace contacts={snapshotData.contacts??[]} events={snapshotData.events??[]} leads={snapshotData.leads??[]} services={snapshotData.services??[]} bookings={snapshotData.bookings??[]} paymentFacts={snapshotData.paymentFacts??[]} batches={snapshotData.batches??[]} items={snapshotData.items??[]} activity={snapshotData.activity??[]} cleanupBatches={cleanupData.batches??[]} cleanupItems={cleanupData.items??[]} cleanupEnabled={!cleanup.error} warning={cleanup.error?"Cleanup history could not be loaded. Cleanup controls are disabled.":null}
    manageContactAction={manageContactAction} manageEventAction={manageEventAction} manageLeadAction={manageLeadAction} stageManifestAction={stageManifestAction} approveBatchAction={approveBatchAction} applyBatchAction={applyBatchAction} compensateBatchAction={compensateBatchAction} previewCleanupAction={previewCleanupAction} archiveCleanupCustomersAction={archiveCleanupCustomersAction} restoreCleanupCustomersAction={restoreCleanupCustomersAction} quarantineCleanupOutboxAction={quarantineCleanupOutboxAction} restoreCleanupOutboxAction={restoreCleanupOutboxAction} previewVeraWarrenAction={previewVeraWarrenAction} applyVeraWarrenAction={applyVeraWarrenAction} compensateVeraWarrenAction={compensateVeraWarrenAction} previewWarrenOwnerAttestationAction={previewWarrenOwnerAttestationAction} applyWarrenOwnerAttestationAction={applyWarrenOwnerAttestationAction} compensateWarrenOwnerAttestationAction={compensateWarrenOwnerAttestationAction}/>;
}
