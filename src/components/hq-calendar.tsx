...
export function HqCalendar({ events, todayKey, unscheduledCount, teamMembers, availability, assignments, currentTeamMemberId, canManageTeam, canManageAssignments, upsertAvailabilityAction, removeAvailabilityAction, manageAssignmentAction, manageTeamMemberAction }: Props) {
  ...
  return <div className="calendar-foundation">
    <div className="calendar-toolbar" role="group" aria-label="Calendar controls">
      <div className="calendar-month-control"><span className="eyebrow">Calendar</span><h2 aria-live="polite">{monthTitle(monthKey)}</h2><div className="calendar-navigation" role="group" aria-label="Month navigation"><button type="button" onClick={() => setMonthKey(moveMonth(monthKey, -1))} aria-label="Previous month">Previous</button><button type="button" onClick={() => chooseDate(todayKey)}>Today</button><button type="button" onClick={() => setMonthKey(moveMonth(monthKey, 1))} aria-label="Next month">Next</button></div></div>
      <div className="calendar-toolbar-actions"><div className="calendar-view-switch" role="group" aria-label="Calendar scope"><button className={scope === "master" ? "active" : ""} onClick={() => setScope("master")} type="button">Master calendar</button><button className={scope === "mine" ? "active" : ""} onClick={() => setScope("mine")} type="button">My schedule</button></div><div className="calendar-view-switch" role="group" aria-label="Calendar view"><button className={view === "month" ? "active" : ""} onClick={() => setView("month")} type="button">Month</button><button className={view === "agenda" ? "active" : ""} onClick={() => setView("agenda")} type="button">Upcoming</button></div><button className={`calendar-filter-toggle${activeFilterCount ? " active" : ""}`} type="button" aria-expanded={filtersOpen} aria-controls="calendar-filters" onClick={() => setFiltersOpen((open) => !open)}>Filters{activeFilterCount ? ` (${activeFilterCount})` : ""}</button><button className="calendar-quick-add" type="button" onClick={() => startEntry()}>Add entry</button></div>
    </div>

    {view === "month" ? <div className="calendar-primary-layout"><section className="calendar-month panel" aria-labelledby="calendar-month-title"><header className="calendar-month-status"><h2 className="sr-only" id="calendar-month-title">{monthTitle(monthKey)}</h2><span className="status-dot">Event-local dates</span><span>{visibleEvents.length + visibleAvailability.length} visible items</span></header><div className="calendar-weekdays" aria-hidden="true">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{days.map((day) => {
      const dayEvents = eventsByDate.get(day.key) ?? [];
      const dayAvailability = availabilityByDate.get(day.key) ?? [];
      const totalItems = dayEvents.length + dayAvailability.length;
      const hiddenCount = Math.max(0, totalItems - 3);
      return <article key={day.key} className={`calendar-day${day.inMonth ? "" : " outside"}${day.key === selectedDate ? " selected" : ""}${day.key === todayKey ? " today" : ""}`}><button type="button" className="calendar-day-number" onClick={() => totalItems ? selectDay(day.key) : startEntry(day.key)} aria-label={totalItems ? `Select ${dateTitle(day.key)}` : `Add calendar entry on ${dateTitle(day.key)}`} aria-pressed={day.key === selectedDate}>{day.day}</button>{day.inMonth ? <span className="calendar-day-state">{totalItems ? `${totalItems} item${totalItems === 1 ? "" : "s"}` : ""}</span> : null}<div className="calendar-day-events">{dayEvents.slice(0, 3).map((event) => <EventSummary event={event} assignments={visibleAssignments} members={members} hasConflict={conflictEventIds.has(event.id)} compact key={event.id} />)}{dayAvailability.slice(0, Math.max(0, 3 - dayEvents.length)).map((entry) => entry.canEdit ? <button type="button" className={`calendar-availability-compact availability-${entry.entryType}`} onClick={() => openEntry(entry)} key={entry.id} aria-label={`View or edit ${teamAvailabilityLabel(entry)} for ${members.get(entry.teamMemberId)?.displayName ?? "team member"}`}><b>{members.get(entry.teamMemberId)?.displayName ?? "Team"}</b><span>{availabilityTypeLabel(entry.entryType)} · {teamAvailabilityLabel(entry)}{conflicted.has(entry.id) ? " · Conflict" : ""}</span></button> : <span className={`calendar-availability-compact availability-${entry.entryType}`} key={entry.id}><b>{members.get(entry.teamMemberId)?.displayName ?? "Team"}</b><span>{availabilityTypeLabel(entry.entryType)} · {teamAvailabilityLabel(entry)}{conflicted.has(entry.id) ? " · Conflict" : ""}</span></span>)}{hiddenCount ? <button className="calendar-more" type="button" onClick={() => selectDay(day.key)}>+{hiddenCount} more</button> : null}</div></article>;
    })}</div></section>
      <section className="date-checker panel calendar-selected-day" id="selected-day-agenda" tabIndex={-1} aria-labelledby="date-checker-title"><header className="panel-heading"><div><span className="eyebrow">Selected day</span><h2 id="date-checker-title">{dateTitle(selectedDate)}</h2></div><button className="primary-button" type="button" onClick={() => startEntry(selectedDate)}>Add entry</button></header><span className={`availability-badge ${selectedAvailability.state}`}>{selectedAvailability.booked.length ? selectedAvailability.label : "No booked event"}</span>{selectedAvailability.booked.map((event: CalendarEvent) => <EventSummary event={event} assignments={assignments} members={members} hasConflict={conflictEventIds.has(event.id)} key={event.id} />)}{selectedAvailability.inquiries.length ? <div className="date-check-results inquiry"><h3>Inquiries / tentative dates</h3>{selectedAvailability.inquiries.map((event: CalendarEvent) => <EventSummary event={event} assignments={assignments} members={members} hasConflict={conflictEventIds.has(event.id)} key={event.id} />)}</div> : null}{(availabilityByDate.get(selectedDate) ?? []).map((entry) => <AvailabilitySummary entry={entry} member={members.get(entry.teamMemberId)} conflict={conflicted.has(entry.id)} onOpen={openEntry} key={entry.id} />)}{!selectedAvailability.booked.length && !selectedAvailability.inquiries.length && !(availabilityByDate.get(selectedDate) ?? []).length ? <div className="calendar-empty-day"><p>No entries for this date.</p><button className="secondary-button" type="button" onClick={() => startEntry(selectedDate)}>Add entry</button></div> : null}<p className="panel-note">No booked event is not a promise that the team, services, travel window, or equipment are available.</p></section>
