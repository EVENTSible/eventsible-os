...
export function HqCalendar({ events, todayKey, unscheduledCount, teamMembers, availability, assignments, currentTeamMemberId, canManageTeam, canManageAssignments, upsertAvailabilityAction, removeAvailabilityAction, manageAssignmentAction, manageTeamMemberAction }: Props) {
  ...
  return <div className="calendar-foundation">
    ...
    {view === "month" ? <section className="calendar-month panel" aria-labelledby="calendar-month-title"><header className="panel-heading"><div><span className="eyebrow">Month</span><h2 id="calendar-month-title">{monthTitle(monthKey)}</h2></div><span className="status-dot">Event-local dates</span></header><div className="calendar-weekdays" aria-hidden="true">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{days.map((day) => {
      ...
      return <article key={day.key} className={`calendar-day${day.inMonth ? "" : " outside"}${day.key === selectedDate ? " selected" : ""}`}><button type="button" className="calendar-day-number" onClick={() => startEntry(day.key)} aria-label={`Add calendar entry on ${dateTitle(day.key)}`}>{day.day}</button>{day.inMonth ? <span className="calendar-day-state">{dayEvents.length + dayAvailability.length ? `${dayEvents.length + dayAvailability.length} item${dayEvents.length + dayAvailability.length === 1 ? "" : "s"}` : ""}</span> : null}<div className="calendar-day-events">{dayEvents.slice(0, 2).map((event) => <EventSummary event={event} assignments={visibleAssignments} members={members} hasConflict={conflictEventIds.has(event.id)} compact key={event.id} />)}{dayAvailability.slice(0, 2).map((entry) => entry.canEdit ? <button type="button" className={`calendar-availability-compact availability-${entry.entryType}`} onClick={() => openEntry(entry)} key={entry.id} aria-label={`View or edit ${teamAvailabilityLabel(entry)} for ${members.get(entry.teamMemberId)?.displayName ?? "team member"}`}><b>{members.get(entry.teamMemberId)?.displayName ?? "Team"}</b><span>{availabilityTypeLabel(entry.entryType)} · {teamAvailabilityLabel(entry)}{conflicted.has(entry.id) ? " · Conflict" : ""}</span></button> : <span className={`calendar-availability-compact availability-${entry.entryType}`} key={entry.id}><b>{members.get(entry.teamMemberId)?.displayName ?? "Team"}</b><span>{availabilityTypeLabel(entry.entryType)} · {teamAvailabilityLabel(entry)}{conflicted.has(entry.id) ? " · Conflict" : ""}</span></span>)}{dayEvents.length + dayAvailability.length > 4 ? <button type="button" onClick={() => { chooseDate(day.key); setView("agenda"); }}>+{dayEvents.length + dayAvailability.length - 4} more</button> : null}</div></article>;
